; !function () {
    ...
    Page.prototype.view = function () {
        ...
        conf.totalCount = conf.totalCount || 0;
        conf.pages = conf.pages | 0;
        conf.curr = (conf.curr | 0) || 1;
        conf.groups = 'groups' in conf ? (conf.groups | 5) : 5;
        //conf.first = '&#x9996;&#x9875;';
        ...
        conf.last = 'last';
        conf.prev = '<span class="icon-arrow-left-large"></span>'
        conf.prev = 'prev' in conf ? conf.prev : '<span class="icon-arrow-left-large"></span>';
        conf.next = '<span class="icon-arrow-right-large"></span>';
        conf.next = 'next' in conf ? conf.next : '<span class="icon-arrow-right-large"></span>';
        ...
        //当前页非首页，则输出上一页
        if (conf.curr > 0 && conf.prev) {
            if (conf.curr == 1) {
                view.push('<span class="prev disabled">' + conf.prev + '</span>');
            } else {
                view.push('<a href="javascript:;" class="prev" data-page="' + (conf.curr - 1) + '">' + conf.prev + '</a>');
            }
        }

        //当前组非首组，则输出首页
        if (dict.index > 1 && conf.first && conf.groups !== 0) {
            view.push('<a href="javascript:;" class="laypage_first" data-page="1"  title="&#x9996;&#x9875;">' + conf.first + '</a><span class="page_ellips">&#x2026;</span>');
        }
        ...
        for (; dict.start <= dict.end; dict.start++) {
            if (dict.start === conf.curr) {
                view.push('<span class="active"' + (/^#/.test(conf.skin) ? 'style="background-color:' + conf.skin + '"' : '') + '>' + dict.start + '</span>');
            } else {
        ...
        if (conf.pages > conf.groups && dict.end < conf.pages && conf.last && conf.groups !== 0) {
            view.push('<span class="page_ellips">&#x2026;</span><a href="javascript:;" class="laypage_last" title="&#x5C3E;&#x9875;"  data-page="' + conf.pages + '">' + conf.last + '</a>');
        }
        ...
        if (conf.curr !== conf.pages && conf.next || dict.flow) {
            view.push((function () {
                ...
                : '<a href="javascript:;" class="next" data-page="' + (conf.curr + 1) + '">' + conf.next + '</a>';
            }()));
        } else {
            view.push('<span class="next disabled">' + conf.next + '</span>');
        }

        //加入总页数
        /*   var totalCountHtml = '共0页';
        $('#pagerinfo').html(totalCountHtml.replace(/\d+/, conf.pages));
        $('#pagerinfo').show();
        if (json.total === 1) {
            $('#pagerinfo').hide();
        }*/
        ...
        }(conf.skin)) : 'default') + '" id="laypage_' + that.config.item + '">' + '<span class="pcPageSpan" style="font-family: Arial,sans-serif,Microsoft YaHei;float: left;font-size: 12px;margin: 0 5px;">' + conf.totalCount + ' In Total,' + conf.pages + ' Pages</span>' + view.join('') + function () {
            return conf.skip
